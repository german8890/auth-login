...
func (m *Masker) Marshal(body interface{}) (interface{}, error) {
	if reflect.TypeOf(body).Kind() == reflect.Ptr {
		if reflect.ValueOf(body).Elem().Kind() == reflect.Slice {
			var newSlice []interface{}
			s := reflect.ValueOf(body).Elem()
			for i := 0; i < s.Len(); i++ {
				body, err := Struct(s.Index(i).Interface())
				if err != nil {
					return nil, err
				}
		...
		if reflect.TypeOf(body).Elem().Kind() == reflect.Struct {
			data, err := Struct(body)
			return data, err
		}
	}
...
func structSlice(val reflect.Value) (interface{}, error) {
	var newSlice []interface{}
	for i := 0; i < val.Len(); i++ {
		body, err := Struct(val.Index(i).Interface())
		if err != nil {
			return nil, err
		}
		newSlice = append(newSlice, body)
	...
	for i := 0; i < selem.NumField(); i++ {
		...
		case reflect.Interface:
			...
			if reflect.TypeOf(selem.Field(i).Interface()).Kind() != reflect.Ptr {
				switch reflect.TypeOf(selem.Field(i).Interface()).Kind() {
				case reflect.Slice:
					if selem.Field(i).IsNil() {
						continue
					}
					/*if selem.Field(i).Type().Kind() == reflect.String {
						orgval := selem.Field(i).Interface().([]string)
						newval := make([]string, len(orgval))
						for i, val := range selem.Field(i).Interface().([]string) {
							newval[i] = m.String(mtype(mtag), val)
					...
					if selem.Field(i).Type().Kind() == reflect.Struct && mtype(mtag) == MStruct {
						newval := reflect.MakeSlice(selem.Field(i).Type(), 0, selem.Field(i).Len())
						for j, l := 0, selem.Field(i).Len(); j < l; j++ {
							_n, err := m.Struct(selem.Field(i).Index(j).Interface())
							if err != nil {
					...
					if selem.Field(i).Type().Kind() == reflect.Ptr && mtype(mtag) == MStruct {
						newval := reflect.MakeSlice(selem.Field(i).Type(), 0, selem.Field(i).Len())
						for j, l := 0, selem.Field(i).Len(); j < l; j++ {
							_n, err := m.Struct(selem.Field(i).Index(j).Interface())
							if err != nil {
					...
					/* Nooo -<
					if selem.Field(i).Type().Kind() == reflect.Struct && mtype(mtag) == MStruct {
						newval := reflect.MakeSlice(selem.Field(i).Elem().Type(), 0, selem.Field(i).Elem().Len())
						for j, l := 0, selem.Field(i).Elem().Len(); j < l; j++ {
							_n, err := m.Struct(selem.Field(i).Elem().Index(j).Interface())
							if err != nil {
								return nil, err
							}
							if reflect.TypeOf(selem.Field(i).Elem().Index(j).Interface()).Kind() != reflect.Ptr {
								newval = reflect.Append(newval, reflect.ValueOf(_n).Elem())
							} else {
								newval = reflect.Append(newval, reflect.ValueOf(_n))
							}
					...
					*/
					if selem.Field(i).Type().Kind() == reflect.Interface && mtype(mtag) == MStruct {
...
// Password always return "************"
func (m *Masker) File(i string) string {
	l := len([]rune(i))
	enc := i[0:5] + "*****" + i[(len(i)-6):(len(i)-1)]
	if l == 0 {
		return ""
	}
	//return strLoop(instance.mask, len("************"))
	return fmt.Sprintf("File base64 length: [%d], detail: [%s]", l, enc)
}
...
func Marshal(s interface{}) (interface{}, error) {
	return instance.Marshal(s)
}
